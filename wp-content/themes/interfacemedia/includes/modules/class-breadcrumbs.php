...
if ( ! class_exists( 'ALT_Breadcrumbs' ) ) {
	...
	class ALT_Breadcrumbs {
		...
		function __construct( $args = array() ) {
			...
			$defaults = array(
				...
				'date_labels'       => array(),
				// team editing start
				'action'            => 'alt_breadcrumbs_render',
				'css_namespace'     => array(),
				'path_type'         => 'full',
				// team editing end
				'post_taxonomy'     => apply_filters(
					'alt_breadcrumbs_trail_taxonomies',
					array(
						'post'      => 'category',
					)
				),
			...
			$this->args = apply_filters( 'alt_breadcrumb_args', wp_parse_args( $args, $defaults ) );
		...
		public function get_trail() {
			...
			$wrapper_classes = apply_filters( 'alt_breadcrumbs_wrapper_classes', $wrapper_classes );
		...
		public function get_items() {
			...
			$breadcrumb = apply_filters( 'alt_breadcrumbs_trail', $breadcrumb, $this->args );
		...
		public function home_title() {
			...
			$title            = esc_html__( 'Home', 'alt' );
			$use_custom_title = apply_filters( 'alt_breadcrumbs_custom_home_title', true );
		...
		public function build_trail() {
			...
			$custom_trail = apply_filters( 'alt_breadcrumbs_custom_trail', false, $this->args );
			 ...
			 */
			$this->items = apply_filters( 'alt_breadcrumbs_items', $this->items, $this->args, $this );
		...
		public function prepare_label( $label, $fallback = false ) {
			...
			if ( false !== $fallback ) {
				$fallback = sprintf(
					apply_filters( 'alt_breadcrumbs_icon_fallback', '<span class="hidden">%s</span>', $fallback ),
					esc_attr( $fallback )
				);
			}
			...
			// Return default icon
			return sprintf(
				apply_filters( 'alt_breadcrumbs_default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
