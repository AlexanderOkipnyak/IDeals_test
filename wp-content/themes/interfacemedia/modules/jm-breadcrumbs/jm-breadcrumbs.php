...
if ( ! class_exists( 'JM_Breadcrumbs' ) ) {
	...
	class JM_Breadcrumbs {
		...
		public $css = array(
			...
			'separator' => 'breadcrumbs-item-sep',
			'link'      => 'breadcrumbs-item-link',
			'target'    => 'breadcrumbs-item-target',
		);
		...
		function __construct( $args = array() ) {
			...
			$defaults = array(
				...
				'date_labels'       => array(),
				// team editing start
				'action'            => 'jm_breadcrumbs_render',
				'css_namespace'     => array(),
				'path_type'         => 'full',
				// team editing end
				'post_taxonomy'     => apply_filters(
					'jm_breadcrumbs_trail_taxonomies',
					array(
						'post'      => 'category',
					)
				),
			...
			$this->args = apply_filters( 'jm_breadcrumb_args', wp_parse_args( $args, $defaults ) );
		...
		public function get_trail() {
			...
			$wrapper_classes = apply_filters( 'jm_breadcrumbs_wrapper_classes', $wrapper_classes );
			...
			if ($breadcrumb) {
				...
				$result .= sprintf( $this->args['wrapper_format'], $breadcrumb );
		...
		public function get_items() {
			...
			$separator = '<span class="' . esc_attr( $this->css['separator'] ) . '">' . $separator . '</span>';
			...
			$breadcrumb = apply_filters( 'jm_breadcrumbs_trail', $breadcrumb, $this->args );
		...
		public function default_labels() {
			...
			$labels = array(
				...
				'error_404'           => esc_html__( '404 Not Found', 'jmwebdesigns' ),
				'archives'            => esc_html__( 'Archives', 'jmwebdesigns' ),
				'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'jmwebdesigns' ),
				'paged'               => esc_html__( 'Page %s', 'jmwebdesigns' ),
				'archive_minute'      => esc_html__( 'Minute %s', 'jmwebdesigns' ),
				'archive_week'        => esc_html__( 'Week %s', 'jmwebdesigns' ),
		...
		public function home_title() {
			...
			$title            = esc_html__( 'Home', 'jmwebdesigns' );
			$use_custom_title = apply_filters( 'jm_breadcrumbs_custom_home_title', true );
		...
		public function build_trail() {
			...
			$custom_trail = apply_filters( 'jm_breadcrumbs_custom_trail', false, $this->args );
			 ...
			 */
			$this->items = apply_filters( 'jm_breadcrumbs_items', $this->items, $this->args, $this );
		...
		public function prepare_label( $label, $fallback = false ) {
			...
			if ( false !== $fallback ) {
				$fallback = sprintf(
					apply_filters( 'jm_breadcrumbs_icon_fallback', '<span class="hidden">%s</span>', $fallback ),
					esc_attr( $fallback )
				);
			}
			...
			// Return default icon
			return sprintf(
				apply_filters( 'jm_breadcrumbs_default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
